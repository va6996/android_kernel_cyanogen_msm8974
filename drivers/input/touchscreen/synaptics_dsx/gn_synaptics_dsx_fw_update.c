		...
		const unsigned char *fw_image)
{
	...
	printk(" tpd header->contains_firmware_id = %d,data->firmware_id[0] = %d,data->firmware_id[1] = %d,data->firmware_id[2] = %d,data->firmware_id[3] = %d",header->contains_firmware_id,data->firmware_id[0],data->firmware_id[1],data->firmware_id[2],data->firmware_id[3]);
...
static enum flash_area fwu_go_nogo(struct image_header_data *header)
{
    ...
    printk(" tpd header->contains_firmware_id = %d,fwu->image_name = %s\n",header->contains_firmware_id,fwu->image_name);
	if (header->contains_firmware_id) {
    ...
    printk(" tpd image_fw_id = %lu,device_fw_id= %u\n",image_fw_id,device_fw_id);
	...
	dev_info(rmi4_data->pdev->dev.parent,
			...
			fwu->config_data[2],
			fwu->config_data[3]);
    printk(" tpd image_config_id = %d,device_config_id= %d\n",image_config_id,device_config_id);
...
static int fwu_do_reflash(void)
{
	int retval;
	struct synaptics_rmi4_data *rmi4_data = fwu->rmi4_data;
    printk(" tpd enter fwu_do_reflash\n");
	retval = fwu_write_bootloader_id();
	if (retval < 0)
		return retval;
...
static int fwu_do_lockdown(void)
{
	...
	if (retval < 0)
		return retval;
    printk(" tpd enter fwu_do_lockdown\n");
	retval = synaptics_rmi4_reg_read(rmi4_data,
			fwu->f34_fd.query_base_addr + fwu->properties_off,
			&fwu->flash_properties,
			sizeof(fwu->flash_properties));
    ...
    printk(" tpd enter fwu_do_lockdown retval = %d,fwu->flash_properties = %d,UNLOCKED = %d\n",retval,fwu->flash_properties,UNLOCKED);
...
static int fwu_start_reflash(void)
{
    ...
    printk(" tpd parse_header suscess! fwu->bl_version = %d,header.bootloader_version = %d\n",fwu->bl_version,header.bootloader_version);
	...
	if (retval < 0)
		goto exit;
    printk(" tpd  fwu_read_f01_device_status retval = %d, f01_device_status.flash_prog = %d\n",retval,f01_device_status.flash_prog);
	if (f01_device_status.flash_prog) {
		dev_info(rmi4_data->pdev->dev.parent,
    ...
    printk(" tpd fwu->do_lockdown = %d,fwu->bl_version = %d\n",fwu->do_lockdown,fwu->bl_version);
	if (fwu->do_lockdown) {
    ...
    printk(" tpd header.firmware_size = %d,header.config_size = %d\n",header.firmware_size,header.config_size);
    ...
    printk(" tpd header.contains_bootloader = %d\n",header.contains_bootloader);
	if (header.contains_bootloader) {
		if (header.firmware_size)
