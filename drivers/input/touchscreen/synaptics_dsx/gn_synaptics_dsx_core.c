...
#define REPORT_2D_Z
#define REPORT_2D_W
//Gionee luliya add for gestures begin 
#define DOUBLE_CLICK_WAKE
#define DRAW_GESTURES
//Gionee luliya add for gestures end
...
#endif
//Gionee luliya 2014-5-30 add for glove begin 
#ifdef TP_GLOVE_SUPPORT
...
#endif
//Gionee luliya 2014-5-30 add for glove end
...
//Gionee luliya 2014-5-30 modify for resume and suspend begin 
#ifdef CONFIG_FB
...
#endif
//Gionee luliya 2014-5-30 modify for resume and suspend end
...
//Gionee luliya 2014-5-30 add for key report begin 
#ifdef DOUBLE_CLICK_WAKE
static void WakeUp_LCD(struct synaptics_rmi4_data *rmi4_data)
{
      printk("tpd WakeUp_LCD\n");
...
#endif
//Gionee luliya 2014-5-30 add for key report end 
...
static void synaptics_rmi4_sensor_report(struct synaptics_rmi4_data *rmi4_data)
{
	...
	if (retval < 0) {
       ...
       if (wake_suspend_compelete) {
           ...
           //gesture U
           if (gesture_switch) {
		   	   printk(" tpd gesture switch enter double_wake_flag[0]= %d,double_wake_flag[2] = %d\n",double_wake_flag[0],double_wake_flag[2]);
               if (double_wake_flag[0] == 0x0b && double_wake_flag[2] == 0x63) {
				   ...
				   //printk(" tpd flag_coordinate[0] = %d,flag_coordinate[1]= %d,flag_coordinate[2]=%d,flag_coordinate[3]=%d\n",flag_coordinate[0],flag_coordinate[1],flag_coordinate[2],flag_coordinate[3]);
...
static int synaptics_rmi4_set_input_dev(struct synaptics_rmi4_data *rmi4_data)
{
	...
	const struct synaptics_dsx_board_data *bdata =
				rmi4_data->hw_if->board_data;
    printk("tpd enter synaptics_rmi4_set_input_dev\n");
...
//	if ((bdata->bus_reg_name != NULL) && (*bdata->bus_reg_name != 0)) {
		rmi4_data->bus_reg = regulator_get(rmi4_data->pdev->dev.parent,
				"vcc_i2c");
        printk(" TPD enter synaptics_rmi4_get_reg \n");
...
static int synaptics_rmi4_suspend(struct device *dev)
{
	...
	int  count = 0;
       ...
       printk(" tpd wake_switch = %d,gesture_switch = %d,cross_switch = %d,character_o_switch = %d\n",wake_switch,gesture_switch,cross_switch,character_o_switch);
#ifdef DOUBLE_CLICK_WAKE
       if (wake_switch || gesture_switch || cross_switch || character_o_switch) {
...
#ifdef TP_GLOVE_SUPPORT
	       printk("tpd glove_enable = %d,glove_switch = %d\n",glove_enable,glove_switch);
           if (glove_enable) {
               do {
                   if_enable_tp_glove(0x00);
		   ...
		   printk("tpd rmi4_data->stay_awake = %d\n",rmi4_data->stay_awake);
           return 0;
       }
#endif
